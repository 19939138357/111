...
vi.mock('../src/middleware/auth.js', () => ({
    requireRole: vi.fn((roles) => {
        return async (request, reply) => {
            request.userId = 'test-librarian-id';
            request.username = 'librarian';
            request.role = UserRole.librarian;
            request.userName = '张馆员';
        };
    }),
}));
const mockAuditLogs = [
...
describe('GET /api/audit-logs - 过滤功能测试', () => {
    ...
    describe('单条件过滤测试', () => {
        it('应能按 actorName 进行模糊过滤', async () => {
            const mockFilteredLogs = mockAuditLogs.filter((log) => log.actor_name.includes('张'));
            vi.mocked(prisma.audit_log.findMany).mockResolvedValueOnce(mockFilteredLogs);
            vi.mocked(prisma.audit_log.count).mockResolvedValueOnce(mockFilteredLogs.length);
            ...
            expect(prisma.audit_log.findMany).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('应能按 targetType 进行精确过滤', async () => {
            const mockFilteredLogs = mockAuditLogs.filter((log) => log.target_type === 'fine_record');
            vi.mocked(prisma.audit_log.findMany).mockResolvedValueOnce(mockFilteredLogs);
            vi.mocked(prisma.audit_log.count).mockResolvedValueOnce(mockFilteredLogs.length);
            const response = await app.inject({
                method: 'GET',
        ...
        it('应能按 action 进行过滤 (现有功能)', async () => {
            const mockFilteredLogs = mockAuditLogs.filter((log) => log.action === AuditAction.book_checked_out);
            vi.mocked(prisma.audit_log.findMany).mockResolvedValueOnce(mockFilteredLogs);
            vi.mocked(prisma.audit_log.count).mockResolvedValueOnce(mockFilteredLogs.length);
            const response = await app.inject({
                method: 'GET',
    ...
    describe('组合过滤测试', () => {
        it('应能同时使用 actorName 和 targetType 进行组合过滤', async () => {
            const mockFilteredLogs = mockAuditLogs.filter((log) => log.actor_name.includes('张') && log.target_type === 'loan_record');
            vi.mocked(prisma.audit_log.findMany).mockResolvedValueOnce(mockFilteredLogs);
            vi.mocked(prisma.audit_log.count).mockResolvedValueOnce(mockFilteredLogs.length);
            const response = await app.inject({
                method: 'GET',
    ...
    describe('默认参数测试', () => {
        it('未提供分页参数时应使用默认值', async () => {
            vi.mocked(prisma.audit_log.findMany).mockResolvedValueOnce(mockAuditLogs.slice(0, 2));
            vi.mocked(prisma.audit_log.count).mockResolvedValueOnce(mockAuditLogs.length);
            const response = await app.inject({
                method: 'GET',
                url: '/api/audit-logs',
            });
